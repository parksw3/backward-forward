...
fit_incubation_truncate_uncertainty_2day <- vector('list', length(tvec))
...
for (i in 1:length(tvec)) {
  ...
  for (j in 1:nsim) {
    ...
    ff1 <- try(fitfun_truncate2(incubationdata3, tmax=tvec[i], logmean=log(true_inc_mean), logshape=0))
    
    tmplist_truncate[[j]] <- ff1
  }
  
  fit_incubation_truncate_uncertainty_2day[[i]] <- tmplist_truncate %>%
    bind_rows(.id="sim")
}

save("fit_incubation_truncate_uncertainty_2day", 
     file="fit_sepir_gamma_uncertainty_2day.rda")
