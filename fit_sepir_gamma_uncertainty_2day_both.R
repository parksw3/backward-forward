...
fit_incubation_truncate_exponential_uncertainty_2day <- vector('list', length(tvec))
...
for (i in 1:length(tvec)) {
  ...
  fit_incubation_truncate_exponential_uncertainty_2day[[i]] <- tmplist_truncate %>%
    bind_rows(.id="sim")
}

save("fit_incubation_truncate_exponential_uncertainty_2day", 
     file="fit_sepir_gamma_uncertainty_2day_both.rda")
