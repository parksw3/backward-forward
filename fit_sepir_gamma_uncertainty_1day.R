...
fit_incubation_truncate_uncertainty_1day <- vector('list', length(tvec))
...
for (i in 1:length(tvec)) {
  ...
  fit_incubation_truncate_uncertainty_1day[[i]] <- tmplist_truncate %>%
    bind_rows(.id="sim")
}

save("fit_incubation_truncate_uncertainty_1day", 
     file="fit_sepir_gamma_uncertainty_1day.rda")
